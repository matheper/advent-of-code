    ...
    if type_id == 4:
        ...
        if length_type_id == "0":
            ...
            while parsed_length < total_length_int:
                previous_i = i
                value, i = parse_packet(packet, versions, i)
                values.append(value)
            ...
            for _ in range(num_sub_packets_int):
                value, i = parse_packet(packet, versions, i)
                values.append(value)
        value = calculate_value(type_id, values)
    return value, i
...
def part_2(input_data):
    packet = "".join([hexa[x] for x in input_data])
    versions = []
    value, _ = parse_packet(packet, versions)
    return value


def main():
    with open("2021/inputs/day_16.txt") as input_file:
