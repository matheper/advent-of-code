...
def find_intersection(cube1, cube2):
    action1, x1, y1, z1 = cube1
    action2, x2, y2, z2 = cube2
    if not (x1[0] <= x2[1] and x1[1] >= x2[0]):
        return
    if not (y1[0] <= y2[1] and y1[1] >= y2[0]):
        return
    if not (z1[0] <= z2[1] and z1[1] >= z2[0]):
        return
...
def part_2(input_data):
    ...
    for step in input_data:
        new_cubes = [step] if step[0] == 1 else []
        for cube in cubes:
            intersection = find_intersection(step, cube)
            if intersection:
                new_cubes.append(intersection)
        cubes += new_cubes

    counter = 0
    for action, (x1, x2), (y1, y2), (z1, z2) in cubes:
        counter += action * (x2 - x1 + 1) * (y2 - y1 + 1) * (z2 - z1 + 1)
    return counter
